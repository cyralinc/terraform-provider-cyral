...
- Set Control Plane configuration environment variables: `CYRAL_TF_CONTROL_PLANE`, `CYRAL_TF_CLIENT_ID`, `CYRAL_TF_CLIENT_SECRET`.
...
2.  Run the migration script (_make sure the script has the required permissions as mentioned in the requirements section_). Please read all instructions carefully while interacting with the script.
