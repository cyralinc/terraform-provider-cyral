...
func TestAccRepositoryNetworkAccessPolicyResource(t *testing.T) {
	...
	emptyFieldsNotEnabled := &network.NetworkAccessPolicy{
		Enabled: false,
		NetworkAccessRules: network.NetworkAccessRules{
			RulesBlockAccess: false,
			Rules:            []network.NetworkAccessRule{},
		},
	}
	emptyFieldsExceptName := &network.NetworkAccessPolicy{
		NetworkAccessRules: network.NetworkAccessRules{
			Rules: []network.NetworkAccessRule{
				{
					Name: "name-1",
	...
	fullRule := &network.NetworkAccessPolicy{
		NetworkAccessRules: network.NetworkAccessRules{
			Rules: []network.NetworkAccessRule{
				{
					Name:        "name-2",
	...
	fullRuleUpdated := &network.NetworkAccessPolicy{
		NetworkAccessRules: network.NetworkAccessRules{
			Rules: []network.NetworkAccessRule{
				{
					Name:        "name-3",
	...
	fullRuleUpdatedAdditionalRule := &network.NetworkAccessPolicy{
		NetworkAccessRules: network.NetworkAccessRules{
			Rules: []network.NetworkAccessRule{
				{
					Name:        "name-3",
...
func setupRepositoryNetworkAccessPolicyTest(
	resName string,
	nap *network.NetworkAccessPolicy,
	dbAccounts []string,
) resource.TestStep {
...
func formatNetworkAccessPolicyIntoConfig(
	resName, repositoryID string, nap *network.NetworkAccessPolicy, dependsOn []string,
) string {
	var narStr string
