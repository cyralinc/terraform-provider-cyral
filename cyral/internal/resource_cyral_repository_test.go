...
var (
	initialRepoConfig = internal.RepoInfo{
		Name:   utils.AccTestName(utils.RepositoryResourceName, "repo"),
		Type:   internal.MongoDB,
		Labels: []string{"rds", "us-east-2"},
		RepoNodes: []*internal.RepoNode{
			{
				Host: "mongo.local",
				Port: 3333,
			},
		},
		MongoDBSettings: &internal.MongoDBSettings{
			ServerType: internal.Standalone,
		},
	}

	updatedRepoConfig = internal.RepoInfo{
	...
	emptyConnDrainingConfig = internal.RepoInfo{
		Name: utils.AccTestName(utils.RepositoryResourceName, "repo-empty-conn-draining"),
		Type: internal.MongoDB,
		ConnParams: &internal.ConnParams{
			ConnDraining: &internal.ConnDraining{},
		},
		RepoNodes: []*internal.RepoNode{
			{
	...
	connDrainingConfig = internal.RepoInfo{
	...
	mixedMultipleNodesConfig = internal.RepoInfo{
		...
		MongoDBSettings: &internal.MongoDBSettings{
			...
			ServerType:     internal.ReplicaSet,
		},
	}

	allRepoNodesAreDynamic = internal.RepoInfo{
...
func setupRepositoryTest(repo internal.RepoInfo, resName string) resource.TestStep {
...
func repoCheckFuctions(repo internal.RepoInfo, resName string) resource.TestCheckFunc {
	...
	if repo.MongoDBSettings != nil {
		if repo.MongoDBSettings.ServerType == internal.ReplicaSet {
			checkFuncs = append(checkFuncs, []resource.TestCheckFunc{
				resource.TestCheckResourceAttr(resourceFullName,
...
func repoAsConfig(repo internal.RepoInfo, resName string) string {
	config := fmt.Sprintf(`
