	...
	return []idpsaml.GenericSAMLIntegration{
		{
			...
			IdpType:  "idp-type-1",
			Disabled: false,
			IdpDescriptor: &idpsaml.GenericSAMLIdpDescriptor{
			...
			SPMetadata: &idpsaml.GenericSAMLSPMetadata{
				XMLDocument: "xml-document-1",
			},
			Attributes: idpsaml.NewRequiredUserAttributes(
				"first-name-1",
				"last-name-1",
				"email-1",
				"groups-1",
...
func testIntegrationIdPSAMLDataSourceChecks(t *testing.T, resName, nameFilter, typeFilter string) resource.TestCheckFunc {
	...
	testFunctions := []resource.TestCheckFunc{
		...
		resource.TestCheckResourceAttr(dataSourceFullName,
			"idp_list.0.attributes.0.first_name", idpsaml.DefaultUserAttributeFirstName,
		),
		resource.TestCheckResourceAttr(dataSourceFullName,
			"idp_list.0.attributes.0.last_name", idpsaml.DefaultUserAttributeLastName,
		),
		resource.TestCheckResourceAttr(dataSourceFullName,
			"idp_list.0.attributes.0.email", idpsaml.DefaultUserAttributeEmail,
		),
		resource.TestCheckResourceAttr(dataSourceFullName,
			"idp_list.0.attributes.0.groups", idpsaml.DefaultUserAttributeGroups,
		),
	}
...
func filterSAMLIdps(idps []idpsaml.GenericSAMLIntegration, nameFilter, typeFilter string) []idpsaml.GenericSAMLIntegration {
