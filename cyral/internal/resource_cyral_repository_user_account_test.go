...
func TestAccRepositoryUserAccountResource(t *testing.T) {
	// Upgrade test
	onlyRequiredFields := internal.UserAccountResource{
		Name: "name-1",
		AuthScheme: &internal.AuthScheme{
			AWSSecretsManager: &internal.AuthSchemeAWSSecretsManager{
				SecretArn: "secret-arn-1",
			},
		},
	}
	anotherName := internal.UserAccountResource{
		Name: "name-2",
		AuthScheme: &internal.AuthScheme{
			AWSSecretsManager: &internal.AuthSchemeAWSSecretsManager{
				SecretArn: "secret-arn-1",
	...
	anotherAuthScheme := internal.UserAccountResource{
		Name: "name-2",
		AuthScheme: &internal.AuthScheme{
			GCPSecretManager: &internal.AuthSchemeGCPSecretManager{
				SecretName: "secret-name-1",
			},
		},
	}
	withOptionalFields := internal.UserAccountResource{
		Name:             "name-2",
		AuthDatabaseName: "auth-database-name-1",
		Config: &internal.UserAccountConfig{
	...
	withOptionalFieldsUpdated := internal.UserAccountResource{
		Name:             "name-3",
	...
	awsIAM := internal.UserAccountResource{
		Name: "aws-iam-useracc",
		AuthScheme: &internal.AuthScheme{
			AWSIAM: &internal.AuthSchemeAWSIAM{
				RoleARN: "role-arn-1",
			},
		},
	}
	awsSecretsManager := internal.UserAccountResource{
		Name: "aws-sm-useracc",
		AuthScheme: &internal.AuthScheme{
			AWSSecretsManager: &internal.AuthSchemeAWSSecretsManager{
				SecretArn: "secret-arn-1",
	...
	cyralStorage := internal.UserAccountResource{
		Name: "cyral-storage-useracc",
		AuthScheme: &internal.AuthScheme{
			CyralStorage: &internal.AuthSchemeCyralStorage{
				Password: "password-1",
			},
		},
	}
	hashicorpVault := internal.UserAccountResource{
		Name: "hashicorp-vault-useracc",
		AuthScheme: &internal.AuthScheme{
			HashicorpVault: &internal.AuthSchemeHashicorpVault{
				Path:                 "path-1",
				IsDynamicUserAccount: true,
			},
		},
	}
	environmentVariable := internal.UserAccountResource{
		Name: "env-var-useracc",
		AuthScheme: &internal.AuthScheme{
			EnvironmentVariable: &internal.AuthSchemeEnvironmentVariable{
				VariableName: "variable-name-1",
			},
		},
	}
	kubernetesSecret := internal.UserAccountResource{
		Name: "kubesecrets-useracc",
		AuthScheme: &internal.AuthScheme{
			KubernetesSecret: &internal.AuthSchemeKubernetesSecret{
				SecretName: "secret-name-1",
				SecretKey:  "secret-key-1",
			},
		},
	}
	gcpSecretManager := internal.UserAccountResource{
		Name: "gcp-useracc",
		AuthScheme: &internal.AuthScheme{
			GCPSecretManager: &internal.AuthSchemeGCPSecretManager{
				SecretName: "secret-name-1",
...
func setupRepositoryUserAccountTest(resName string, userAccount internal.UserAccountResource) resource.TestStep {
...
func setupRepositoryUserAccountCheck(resName string, userAccount internal.UserAccountResource) resource.TestCheckFunc {
...
func setupRepositoryUserAccountConfig(resName string, userAccount internal.UserAccountResource) string {
	var config string
