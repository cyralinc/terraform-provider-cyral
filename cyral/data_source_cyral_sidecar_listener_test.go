	...
	if repoTypeFilter != "" && portFilter > 0 {
		return fmt.Sprintf(`
			data "cyral_sidecar_listener" "test_listener" {
				...
				repo_type = "%s"
				port = %d
			}`, listToStr(dependsOn), basicSidecarID, repoTypeFilter, portFilter)
		...
		return fmt.Sprintf(`
			data "cyral_sidecar_listener" "test_listener" {
				...
				sidecar_id = %s
				repo_type = "%s"
			}`, listToStr(dependsOn), basicSidecarID, repoTypeFilter)
	} else {
		return fmt.Sprintf(`
			data "cyral_sidecar_listener" "test_listener" {
				depends_on = %s
				sidecar_id = %s
				port = %d
			}`, listToStr(dependsOn), basicSidecarID, portFilter)
	}
}
