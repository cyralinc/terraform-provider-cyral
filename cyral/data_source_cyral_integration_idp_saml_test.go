...
func testIntegrationIdPSAMLDataSourceChecks(t *testing.T, resName, nameFilter, typeFilter string) resource.TestCheckFunc {
	...
	testFunctions := []resource.TestCheckFunc{
		...
		resource.TestMatchResourceAttr(dataSourceFullName,
			"idp_list.0.sp_metadata.0.url", nonEmptyRegex,
		),
		resource.TestMatchResourceAttr(dataSourceFullName,
			"idp_list.0.sp_metadata.0.entity_id", nonEmptyRegex,
		),
		resource.TestMatchResourceAttr(dataSourceFullName,
			"idp_list.0.sp_metadata.0.single_logout_url", nonEmptyRegex,
		),
		resource.TestMatchResourceAttr(dataSourceFullName,
			"idp_list.0.sp_metadata.0.assertion_consumer_services.#", nonEmptyRegex,
		),
		resource.TestMatchResourceAttr(dataSourceFullName,
			"idp_list.0.sp_metadata.0.assertion_consumer_services.0.url", nonEmptyRegex,
		),
		resource.TestMatchResourceAttr(dataSourceFullName,
			"idp_list.0.sp_metadata.0.assertion_consumer_services.0.index", nonEmptyRegex,
		),
		resource.TestMatchResourceAttr(dataSourceFullName,
			"idp_list.0.sp_metadata.0.assertion_consumer_services.1.url", nonEmptyRegex,
		),
		resource.TestMatchResourceAttr(dataSourceFullName,
			"idp_list.0.sp_metadata.0.assertion_consumer_services.1.index", nonEmptyRegex,
		),
		resource.TestCheckResourceAttr(dataSourceFullName,
